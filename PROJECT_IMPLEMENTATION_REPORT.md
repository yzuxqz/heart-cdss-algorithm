...
1. 在 `app.py` 清理历史遗留未执行代码块，降低系统维护复杂度
2. 引入“模型校准曲线/Brier Score”可视化，强化医疗决策的临床可靠性论证
3. 补充统计显著性检验模块（DeLong/McNemar），增强算法对比的学术严谨性
4. 集成 SUS 问卷页面与结果自动聚合（直接支撑论文可用性分析章节）
5. 建立统一的 `config` 配置体系，提升系统解耦度与可移植性
...
- [ ] **系统演示脚本**：固定 1 组手工输入 + 1 个批量 CSV 示例，确保演示过程稳定流畅
- [ ] **技术口径准备**：准备“不同数据集任务定义差异”的相关技术说明，确保实验方案的逻辑自洽性
...
- [ ] **批量高风险筛选**：批量预测后按风险排序并导出 Top-N high-risk 清单
